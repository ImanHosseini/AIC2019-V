...
public class VisionTools {
    ...
    public boolean canHit(Cell startCell,Cell targetCell){
        ArrayList<Cell> path = new ArrayList<>();
        dfs(startCell, startCell, targetCell, new HashMap<>(), path, false);
        return path.contains(targetCell);
    }

    public boolean canHitInPath(Cell startCell,Cell targetCell, Cell midpoint){
        ArrayList<Cell> path = new ArrayList<>();
        dfs(startCell, startCell, targetCell, new HashMap<>(), path, false);
        return path.contains(targetCell);
    }
    ...
    public Set<Cell> getHeroVision(Hero hero) {
        ...
        for (int i = 0; i < map.getRowNum(); i++) {
            for (int j = 0; j < map.getColumnNum(); j++) {
                ...
                if (isInVision(cell, hero.getCurrentCell())) {
                    vision.add(cell);
                }
            }
        }
